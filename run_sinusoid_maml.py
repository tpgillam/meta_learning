...
def main():
    ...
    with Timer() as timer:
        mses = maml_train(module, device, first_order=first_order, verbose=verbose)
    print(f"Time: {timer}")

    # Save the output to disk.
    with open(os.path.join('trained_models', f'sinusoid_maml_{experiment_name}_train_mse.pkl'), 'wb') as file_:
        pickle.dump(mses, file_)
    torch.save(module.state_dict(), f'trained_models/sinusoid_maml_{experiment_name}.pt')
